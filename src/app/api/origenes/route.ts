...
// Competencia por país de origen en un mercado importador (Comtrade): share, precio US$/L y a/a.
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await db().from('mercado_origen').select('origen,anio,valor_usd,litros').eq('mercado', mercado);
  ...
  const filas = cur.map((r: any) => {
    ...
    return { origen: r.origen, usd: Math.round(u), litros: Math.round(l), precio_l: l > 0 ? +(u / l).toFixed(2) : null, share: +(100 * u / total).toFixed(1), delta_pct: p ? +(100 * (u - p) / p).toFixed(1) : null };
